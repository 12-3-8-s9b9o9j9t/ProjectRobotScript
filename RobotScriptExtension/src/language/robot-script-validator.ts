...
export function registerValidationChecks(services: RobotScriptServices) {
    ...
    const checks: ValidationChecks<RobotScriptAstType> = {
        ...
        FunCall: [validator.checkFunCallParamNumber, validator.checkFunCallExprNotVoid],
...
export class RobotScriptValidator {
     ...
     */
    private checkScopeFun(fun: FunDef, accept: ValidationAcceptor, outScope: TreeScope): void {
    ...
    private checkScopeStatement(
        ...
        if (isBlock(stmt)) {
            const scope = isFunDef(stmt.$container) ? outScope : outScope.createChild(stmt)
            ...
            scope.getLocalUnused().forEach((def) => {
                accept(
                    'warning',
                    `Unused ${isFunDef(def.$container) ? 'parameter' : 'variable'} '${def.name}'.`,
                    {
                        node: def,
                        property: 'name',
                        tags: [1],
     ...
     */
    private checkReturnRec(stmt: Statement, accept: ValidationAcceptor, type: AnyType): boolean {
        if (isReturnStmt(stmt)) {
            ...
            if (cond !== undefined) {
                accept(
                    cond ? 'hint' : 'warning',
                    `Loop condition will always evaluate to ${!!cond}.`,
                    {
                        node: stmt.expr,
                        tags: [1],
                    },
                ...
                return (
                    this.checkReturnRec(stmt.stmt, accept, type) && cond !== undefined // if cond is undefined we cannot be sure that the loop will run
                )
            }
        } else if (isIfStmt(stmt)) {
            const cond = getPcval(stmt.expr)
            ...
            if (cond === undefined) {
                ...
                const elseRet =
                    stmt.stmt2 !== undefined && this.checkReturnRec(stmt.stmt2, accept, type)
                ...
                return stmt.stmt2 !== undefined && this.checkReturnRec(stmt.stmt2, accept, type)
            }
        } else if (isBlock(stmt) && stmt.stmts?.length > 0) {
            const retStmt = stmt.stmts.find((stmt) => this.checkReturnRec(stmt, accept, type))
    ...
    checkFunCallParamNumber(call: FunCall, accept: ValidationAcceptor): void {
        if (call.fun.ref?.params && call.fun.ref.name && call.params) {
     ...
     * @param accept Le callback permettant de signaler une erreur
     */
    checkVarDeclInsideBlock(varDecl: VarDecl, accept: ValidationAcceptor): void {
     ...
     * @param accept Le callback permettant de signaler une erreur
     */
    checkFunCallExprNotVoid(call: FunCall, accept: ValidationAcceptor): void {
        const restr = call as UsageRestrictedFunCall
        const container = call.$container
        if (
            isBlock(container) ||
            ((isIfStmt(container) || isWhileStmt(container)) && container.expr !== call)
        ) {
            restr.usage = 'stmt'
        } else {
            restr.usage = 'expr'

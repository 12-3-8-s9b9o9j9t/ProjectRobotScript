    ...
    public generateName(): string {
        ...
        switch (this.suffix.charAt(this.suffix.length - 1)) {
            ...
            default:
                ...
                this.suffix =
                    this.suffix.slice(0, -1) +
                    String.fromCharCode(this.suffix.charCodeAt(this.suffix.length - 1) + 1)
                break
        }
        return name
    }
...
export class Compiler implements RobotScriptVisitor {
    ...
    visitReturnStmt(ret: ReturnStmt): { returned: boolean; text: string } {
        const expr = getPcval(ret.expr as AST.Expression)?.toString() ?? ret.expr?.accept(this)
        const res = expr ? ` ${expr}` : ''
        return {
            returned: true,
